...
namespace dcn::loader
{
    namespace
    {
        ...
        constexpr const char* PT_BUILD_VERSION = "pt-static-ri-event-v2";
        ...
        const std::filesystem::path & storage_path)
    {
        ...
        spdlog::debug("JSON storage import start: storage_path='{}'", storage_path.string());
        ...
        JsonImportContext & context)
    {
        ...
        if(!deploy_result && deploy_result.error().kind != pt::PTDeployError::Kind::TRANSFORMATION_ALREADY_REGISTERED)
        {
            spdlog::error(
                "Failed to import transformation '{}' from JSON into EVM: {}",
                name,
                static_cast<int>(deploy_result.error().kind));
            co_return false;
        }
        ...
        {
            spdlog::debug("JSON import ensure transformation '{}': already on EVM", name);
        }
        ...
        JsonImportContext & context)
    {
        ...
        if(!deploy_result && deploy_result.error().kind != pt::PTDeployError::Kind::CONDITION_ALREADY_REGISTERED)
        {
            spdlog::error(
                "Failed to import condition '{}' from JSON into EVM: {}",
                name,
                static_cast<int>(deploy_result.error().kind));
            co_return false;
        }
        ...
        {
            spdlog::debug("JSON import ensure condition '{}': already on EVM", name);
        }
        ...
        JsonImportContext & context)
    {
        ...
        if(ok)
        {
            ...
            const auto deploy_result = co_await deployConnector(
                ...
                storage_path,
                false);
            if(!deploy_result && deploy_result.error().kind != pt::PTDeployError::Kind::CONNECTOR_ALREADY_REGISTERED)
            {
                spdlog::error(
                    "Failed to import connector '{}' from JSON into EVM: {}",
                    name,
                    static_cast<int>(deploy_result.error().kind));
                ok = false;
            }
            ...
            {
                spdlog::debug("JSON import connector '{}': already on EVM", name);
            }
        }
        ...
        ConnectorEnsureContext & context)
    {
        ...
        if(ok)
        {
            ...
            {
                spdlog::debug("JSON import deploy connector '{}': already on EVM", name);
            }
        }
