...
namespace dcn
{
    ...
    asio::awaitable<http::Response> HEAD_condition(const http::Request & request, std::vector<server::RouteArg> args, server::QueryArgsList, registry::Registry & registry)
    {
        ...
        if (args.size() == 2) {
            ...
            const auto condition_address_result = evmc::from_hex<chain::Address>(*condition_address_arg);
    ...
    asio::awaitable<http::Response> GET_condition(const http::Request & request, std::vector<server::RouteArg> args, server::QueryArgsList, registry::Registry & registry, evm::EVM & evm)
    {
        ...
        // function selector
        const auto selector = crypto::constructSelector("getCondition(string)");
        ...
        const auto condition_address_res = chain::readAddressWord(exec_result.value());
        
        if(!condition_address_res)
        {
            response.setCode(http::Code::InternalServerError)
                .setBodyWithContentLength(json {
                    {"message", "Failed to fetch condition address"}
                }.dump());

            co_return response;
        }

        const auto & condition_address = condition_address_res.value();
