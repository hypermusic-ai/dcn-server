...
namespace dcn::mainnet
{
    ...
    namespace
    {
        ...
        std::optional<evm::Address> _readAddressWord(const std::uint8_t* data, std::size_t data_size, std::size_t offset)
        {
            if(data == nullptr || offset + 32 > data_size)
            {
                return std::nullopt;
        ...
        std::optional<std::string> _decodeAbiString(const std::uint8_t* data, std::size_t data_size, std::size_t string_offset)
        {
            const auto len_res = _readWordAsSizeT(data, data_size, string_offset);
            if(!len_res)
            {
                return std::nullopt;
            }

            const std::size_t len = *len_res;
            if(string_offset + 32 > data_size || len > data_size - (string_offset + 32))
            {
                return std::nullopt;
            }

            return std::string(reinterpret_cast<const char*>(data + string_offset + 32), len);
        }
        ...
        std::optional<SimpleAddedEvent> _decodeSimpleAddedEvent(const std::string & data_hex)
        {
            ...
            const auto caller_res = _readAddressWord(bytes.data(), bytes.size(), 0);
            const auto name_offset_res = _readWordAsSizeT(bytes.data(), bytes.size(), 32);
            const auto entity_res = _readAddressWord(bytes.data(), bytes.size(), 64);
            if(!caller_res || !name_offset_res || !entity_res)
            {
                return std::nullopt;
            }

            const auto name_res = _decodeAbiString(bytes.data(), bytes.size(), *name_offset_res);
            if(!name_res)
            {
                return std::nullopt;
            }
        ...
        std::optional<json> _rpcCallWithCurl(const std::string & rpc_url, const json & request)
        {
            ...
            try
            {
                ...
                if(exit_code != 0)
                {
                    spdlog::error("Mainnet RPC call failed (exit={}): {}", exit_code, output);
                    return std::nullopt;
                }
            ...
            {
                spdlog::error("Mainnet RPC call failed: {}", e.what());
                return std::nullopt;
            }
        }
        ...
        std::optional<json> _rpcResult(const RpcCall & rpc_call, const std::string & rpc_url, const std::string & method, json params)
        {
            ...
            {
                spdlog::error("Mainnet RPC error on '{}': {}", method, (*response)["error"].dump());
                return std::nullopt;
            }

            if(!response->contains("result"))
            {
                spdlog::error("Mainnet RPC malformed response on '{}': missing result", method);
                return std::nullopt;
            }
        ...
        std::optional<json> _ethGetLogs(const RpcCall & rpc_call,
                                        ...
                                        const evm::Address & registry_address,
        ...
        std::optional<evm::Address> _ethGetOwner(const RpcCall & rpc_call, const std::string & rpc_url, const evm::Address & contract_address)
        {
            const auto selector = evm::constructSelector("getOwner()");
        ...
        {
            return cfg.storage_path / "mainnet" / "cursor.json";
        }
        ...
        std::optional<std::uint64_t> _loadNextBlock(const std::filesystem::path & state_path)
        {
            ...
            {
                spdlog::warn("Failed to parse mainnet cursor '{}': {}", state_path.string(), e.what());
            }
        ...
        bool _saveNextBlock(const std::filesystem::path & state_path, const std::uint64_t next_block)
        {
            ...
            {
                spdlog::warn("Failed to write mainnet cursor '{}': {}", state_path.string(), e.what());
                return false;
            }
        }
        ...
        asio::awaitable<bool> _addFeatureRecord(registry::Registry & registry, const std::filesystem::path & storage_path,
                                                const evm::Address & address, const FeatureRecord & record)
        {
        ...
        asio::awaitable<bool> _addTransformationRecord(registry::Registry & registry, const std::filesystem::path & storage_path,
                                                       const evm::Address & address, const TransformationRecord & record)
        {
        ...
        asio::awaitable<bool> _addConditionRecord(registry::Registry & registry, const std::filesystem::path & storage_path,
                                                  const evm::Address & address, const ConditionRecord & record)
        {
        ...
        asio::awaitable<bool> _addParticleRecord(registry::Registry & registry, const std::filesystem::path & storage_path,
                                                 const evm::Address & address, const ParticleRecord & record)
        {
        ...
        bool _isSamePendingParticle(const PendingParticle & item, const evm::Address & address, const std::string & name)
        {
    ...
    asio::awaitable<void> runEventIngestion(IngestionConfig cfg, registry::Registry & registry, IngestionRuntimeOptions runtime_options)
    {
        ...
        {
            spdlog::error("Mainnet sync enabled but RPC URL is empty");
            co_return;
        }
        ...
        if(!rpc_call)
        {
            spdlog::error("Mainnet sync has no RPC call provider");
            co_return;
        }
        ...
        spdlog::info("Mainnet ingestion started for registry {}", evmc::hex(cfg.registry_address));
        ...
        while(true)
        {
            ...
            if(!next_block)
            {
                ...
                spdlog::info("Mainnet ingestion cursor initialized at block {}", *next_block);
            }
